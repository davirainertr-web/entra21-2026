...
public class PedidoDao {
    ...
    public Pedido consultar(int id) {
        ...
        try {
            PreparedStatement stm = con.prepareStatement("select * from tb_pedidos where id = ?");
            stm.setInt(1, id);
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
                Cliente cliente = clienteDao.consultar(rs.getInt("cliente_id"));
                pedido = new Pedido(rs.getInt("id"), cliente,
                        rs.getTimestamp("data").toLocalDateTime(),
                        rs.getString("status"), carregarItens(con, id));
            }
            rs.close();
            stm.close();
            con.close();
    ...
    public List<Pedido> consultar() {
        ...
        try {
            ...
            while (rs.next()) {
                Cliente cliente = clienteDao.consultar(rs.getInt("cliente_id"));
                pedidos.add(new Pedido(rs.getInt("id"), cliente,
                        rs.getTimestamp("data").toLocalDateTime(),
                        rs.getString("status"), carregarItens(con, rs.getInt("id"))));
            }
            rs.close();
            stm.close();
            con.close();
    ...
    private List<ItemPedido> carregarItens(Connection con, int pedidoId) throws SQLException {
        ...
        PreparedStatement stm = con.prepareStatement("select * from tb_itens_pedido where pedido_id = ?");
    ...
    boolean removido = pedido.getItens().removeIf(item -> item.getProduto().getId() == produtoId);
    if (!removido) {
...
public Pedido finalizarPedido(Pedido pedido) {
    ...
    try {
        ...
        PreparedStatement stmPedido = con.prepareStatement(sqlPedido, Statement.RETURN_GENERATED_KEYS);
        ...
        for (ItemPedido item : pedido.getItens()) {
            produtoDao.baixarEstoque(item.getProduto().getId(), item.getQuantidade());
        }
